...
# Functions to handle adding entries and rerun safely
def add_joy10_entry():
    new_joy10 = st.session_state.get("new_joy10", "").strip()
    if new_joy10:
...
def add_big_lever_entry():
    new_big_lever = st.session_state.get("new_big_lever", "").strip()
    if new_big_lever:
...
def add_now_entry():
    new_now = st.session_state.get("now_text", "").strip()
    dur = st.session_state.get("now_duration", "once")
    if new_now:
...
with right_col:
    ...
    if st.button("Add Big Lever"):
        if add_big_lever_entry():
            st.experimental_rerun()
    ...
    now_duration = st.selectbox("Duration", options=["once", "daily", "weekly"], index=["once", "daily", "weekly"].index(st.session_state.now_duration), key="now_duration")
    if st.button("Add Joy Chore"):
        if add_now_entry():
            st.experimental_rerun()
